...
income = income.rename(
    columns={"SourceOfIncome": "Source of Income", "MonthlyIncome": "Monthly Income"}
)
...
concern = concern.rename(
    columns={"ConcernType": "Concern Type", "ConcernDetails": "Concern Detail"}
)
...
if not senior.empty:
    ...
    for index, row in senior.iterrows():
        ...
        with col1:
            ...
            st.write(
                f"**Name of Spouse:** {row['SpouseName'] if pd.notna(row['SpouseName']) and row['SpouseName'] != '' else 'N/A'}"
            )
            st.write(f"**Father's Name:** {row['FatherName']}")

        with col2:
            st.write(
                f"**Email:** {row['ActiveEmailAddress'] if pd.notna(row['ActiveEmailAddress']) and row['ActiveEmailAddress'] != '' else 'N/A'}"
            )
            st.write(
                f"**Phone Number:** {row['PrimaryContactNum'] if pd.notna(row['PrimaryContactNum']) and row['PrimaryContactNum'] != '' else 'N/A'}"
            )
            st.write(f"**Age:** {age}")
